...
\newglossaryentry{locg}{
    ...
    text={LOC},
    sort=loc,
    description={A general software-engineering metric used to quantify the size or complexity of a software program by counting the number of lines in its source code. LOC is often used as an indicator of development effort, code maintenance, and project scale.}
}
...
\newglossaryentry{casg}{ 
 ...
 text={CAS},
 sort=cas,
 description={One of the custom metrics defined for this thesis project. It measures the percentage of components that are accessible by default without requiring additional developer intervention. This score reflects the baseline accessibility support provided by a framework before any explicit accessibility enhancements are applied}
}
...
\newglossaryentry{cifg}{ 
 ...
 text={CIF},
 sort=cif,
 description={One of the custom metrics defined for this thesis project. It is a weighted measure of implementation complexity that considers not just code volume but also structural factors such as nesting depth, dependency requirements, and property count. CIF is calculated using the formula: CIF = (IMO/TC) × CF, where TC is total component code and CF is a complexity factor based on implementation characteristics}
}
...
\newglossaryentry{dteg}{ 
 ...
 text={DTE},
 sort=dte,
 description={One of the custom metrics defined for this thesis project. It quantifies the estimated development time required to implement accessibility features, calculated by combining empirical measurements with complexity-based adjustments. DTE provides a practical measure of the real-world effort required for accessibility implementation}
}