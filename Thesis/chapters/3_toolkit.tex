...
React Native integrates accessibility features directly into its component model. Developers can attach accessibility properties to components in much the same way as ARIA attributes are added to HTML elements in web development. Key accessibility properties include:
\begin{itemize}
    \item \texttt{accessibilityLabel}: Provides a text description that is read by screen readers;
    \item \texttt{accessibilityRole}: Defines the role (e.g., \texttt{"button"}, \texttt{"header"}) of an element, aiding assistive technologies in interpreting it correctly;
    \item \texttt{accessibilityHint}: Offers additional context about an element's function.
    \item \texttt{accessibilityState}: Conveys the current state of an element (for example, whether it is selected or disabled).
\end{itemize}

\subsection{Advantages and developer benefits}
Using React Native offers several benefits for developers:
\begin{itemize}
...
\begin{itemize}
    ...
    In native development, accessibility is handled through platform-specific APIs—\gls{voiceover} on iOS and \gls{talkback} on Android—which require different tools and approaches. React Native provides a unified \acrshort{api}, streamlining the implementation of accessibility features across both platforms.
...
\begin{itemize}
    ...
    To support users with reduced dexterity, touch targets (buttons and cards) are designed to be at least 24\,px by 24\,px.
\end{itemize}
...
\label{subsubsec:home-reactnative-impl}

The following refined code snippet illustrates how the above success criteria are applied in the Home Screen. In addition, several key accessibility metrics are computed to provide real-time feedback on compliance:
